...
class Distiller:
    def __init__(
        ...
        with open ("./step.txt","w") as f:
            f.write(f"epoch 당 step 수 (batch 수) : {self.num_steps_epoch}")
        ...
        if self.multi_gpu:
            if self.fp16:
                ...
                print("@"*50)
                print("READY FOR TRAIN (apex DDP)")
            else:
                ...
                print("@"*50)
                print("READY FOR TRAIN (torch DDP)")
    ...
    def train(self):
        ...
        self.student.train() #배우는 학생
        self.teacher.eval() #알려주는 선생님

        for _ in range(self.params.n_epoch):
            if self.is_master:
                logger.info(f"--- Starting epoch {self.epoch}/{self.params.n_epoch-1}")
            if self.multi_gpu:
                torch.distributed.barrier() #모든 프로세스가 이 함수 라인에 올 때까지 동기화 (wait)
    ...
    def step(self, input_ids: torch.tensor, attention_mask: torch.tensor, lm_labels: torch.tensor):
        ...
        #mask가 true인 곳만을 모아서 1D tensor로 반환
        s_logits_slct = torch.masked_select(s_logits, mask)  # (bs * seq_length * voc_size) modulo the 1s in mask
        #(batch*seq, vocab_size)로 shape 변환
        ...
        loss_ce = (
            self.ce_loss_fct(
                nn.functional.log_softmax(s_logits_slct / self.temperature, dim=-1), #softmax 후에 log 적용
        ...
        if self.alpha_cos > 0.0:
            ...
            #torch 상에서 1-cos(x1,x2) 에 대한 cosine distance를 적용하려면 y 값 (target) 이 1이어야함
            #근데 왜 new함수까지 써가면서 복사를 할까? 답 : 동일한 device, data type을 갖는 텐서를 만들기 위함
        ...
        self.n_sequences_epoch += input_ids.size(0) #epoch당 배치(문장, 시퀀스) 수
