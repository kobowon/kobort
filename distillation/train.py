...
MODEL_CLASSES = {
    ...
    "roberta": (RobertaConfig, RobertaForMaskedLM, BertTokenizer),
...
def main():
    ...
    parser.add_argument("--teacher_name_or_path", type=str, required=True, help="The teacher model.")
    ...
    set_seed(args)
    #아래 키워드는 utils.init_gpu_params에 정의되어 있음
    if args.is_master:
    ...
    # TOKENIZER #
    tokenizer = teacher_tokenizer_class.from_pretrained(args.teacher_name_or_path,
                                              do_lower_case=False,
                                              unk_token='<unk>',
                                              sep_token='</s>',
                                              pad_token='<pad>',
    ...
    # DISTILLER #
    torch.cuda.empty_cache() #GPU 캐시 상에 남는 데이터 삭제
    distiller = Distiller(
